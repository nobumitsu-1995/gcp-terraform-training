...
for svc in order-svc processing-svc notify-svc; do
  (cd examples/${svc} \
   && docker build -t asia-northeast1-docker.pkg.dev/${PROJECT}/training-repo/${svc}:v1 . \
   && docker push   asia-northeast1-docker.pkg.dev/${PROJECT}/training-repo/${svc}:v1)
done

# 2. Terraform apply（10〜15分かかります — Cloud SQL の作成待ち）
cd examples/terraform
terraform init
terraform apply -var="project_id=${PROJECT}"

# 3. API Gateway の URL を取得
...
# 5. Cloud Run のログで各サービスが順に動いていることを確認
...
# 6. 検証完了後は必ず destroy（Cloud SQL は時間課金）
