...
# 2. Functions のソースコードをZIP化
cd docs/gcp/day08_workshop_level2/examples/functions
zip -r ../terraform/csv-trigger.zip .
cd -  # 元のディレクトリに戻る

# 3. ETL Worker のコンテナイメージをビルド & プッシュ
# (Day6で作成したリポジトリを再利用します)
cd docs/gcp/day08_workshop_level2/examples/etl-worker
gcloud auth configure-docker asia-northeast1-docker.pkg.dev
docker build -t asia-northeast1-docker.pkg.dev/${GOOGLE_CLOUD_PROJECT}/training-repo/etl-worker:v1 .
docker push   asia-northeast1-docker.pkg.dev/${GOOGLE_CLOUD_PROJECT}/training-repo/etl-worker:v1
cd -

# --- Terraform デプロイ ---
# 4. Terraform apply
cd docs/gcp/day08_workshop_level2/examples/terraform
terraform init
terraform apply

# --- 動作確認 ---
# 5. CSV を Raw バケットにアップロードしてパイプラインをトリガー
...
gsutil cp sample.csv gs://${GOOGLE_CLOUD_PROJECT}-raw-csv/
```

数十秒待ってから、BigQueryでデータを確認します。
